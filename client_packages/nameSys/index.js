    ...
    mp.events.add({
      ...
      'entityStreamIn': (entity) => {
        ...
        if(mp.players.local.getVariable('devdebug') == true) {
          mp.gui.chat.push(`entity Type: ${entity.type} Id: [${entity.remoteId}] was streamed.`)
      ...
      'entityStreamOut': (entity) => {
        if(mp.players.local.getVariable('devdebug') == true) {
          mp.console.logInfo(`${JSON.stringify(entity)}`, true, true);
          mp.gui.chat.push(`entity Type: ${entity.type} Id: [${entity.remoteId}] was streamed out.`)
        }
      },
          ...
          if(mp.players.local.getVariable('devdebug')) {
            ...
            if(distance <= maxDistance) {
                ...
                if(tabbed && !player.isTypingInTextChat) {
                  graphics.drawText(`~HUD_COLOUR_PURPLELIGHT~Tabbed out~w~`, [x, (y / 1.1)],
                  {
                    font: 4,
                ...
                if(player.getVariable('talkMsg')) {
                  graphics.drawText(`${player.getVariable('talkMsg')}`, [x, (y / 1.2)],
                  {
                  font: 4,
                ...
                if(player.getVariable('ameMsg') != null) {
                  graphics.drawText(`${player.getVariable('ameMsg')}`, [x, (y * 1.1)],
                  {
                    font: 4,
                ...
                if(mp.players.local.getVariable('adminDuty') == true && mp.players.local.getVariable('adminFly') == true) {return;}

                if(player.getVariable('talking') == true) {
                  graphics.requestStreamedTextureDict("mplobby", true);
                  graphics.drawSprite("mplobby", "mp_charcard_stats_icons9", x, y * 1.3, 0.012, 0.012, 0, 255, 255, 255, 255);
                }

                if(onaduty) {
                  graphics.drawText(`~w~<font color="#ca75ff">[Staff]</font> ${aname}`, [x, y],
                  {
                    font: 4,
                ...
                if(mp.players.local.aliasTog) {
                  graphics.drawText(`${player.getVariable('totalPlayTime') && player.getVariable('totalPlayTime') < 2880 ? "~g~(New Player)~w~~n~" : ''} ~HUD_COLOUR_PURPLELIGHT~#${sid}~w~`, [x, y],
                  {
                    font: 4,
                    color: [255, 255, 255, 255],
                    scale: [0.3, 0.3],
                ...
                if(!mp.players.local.aliasTog) {
                  graphics.drawText(`${player.getVariable('totalPlayTime') && player.getVariable('totalPlayTime') < 2880 ? "~g~(New Player)~w~~n~" : ''} ${player.nickName ? `~HUD_COLOUR_PURPLELIGHT~#${sid}~w~ ${player.nickName}` : `~HUD_COLOUR_PURPLELIGHT~#${sid}~w~`}`, [x, y],
                  {
                    font: 4,
                    color: [255, 255, 255, 255],
                    scale: [0.3, 0.3],
                ...
                if(player.getVariable('injured') == true) {
                  ...
                  graphics.drawSprite("mpinventory", "survival", x, y / 1.009, 0.023, 0.023, 0, 255, 117, 117, 255);
                }
            }
        })
      },
      'ameCreate': (message) => {
        ...
        this.ameNot = setInterval(() => {
          if(this.opacity < 0) { clearInterval(this.ameNot) }
          if(this.x > 0.62) {
            mp.players.local.ameMsg = null;
            clearInterval(this.ameNot)
          }
