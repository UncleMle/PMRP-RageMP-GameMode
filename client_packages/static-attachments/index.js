mp.attachmentMngr = {
    ...
    addFor: function(entity, id) {
        if(entity.getVariable('adminDuty')) { return; }
        if (!entity.__attachmentObjects) {entity.__attachmentObjects = {};}
        if (this.attachments.hasOwnProperty(id) && entity) {
            ...
            mp.gui.chat.push(id.toString(16))
    ...
    register: function(id, model, boneName, offset, rotation) {
        ...
        if (!this.attachments.hasOwnProperty(id)) {
            if (mp.game.streaming.isModelInCdimage(model)) {
                ...
                mp.gui.chat.push(model.toString(16))
            }
        } else {
            mp.gui.chat.push('Duplicate Entry');
        }
    },
